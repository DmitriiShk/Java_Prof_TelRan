...
public class StreamTakeWhileExample {
    ...
    public static void main(String[] args) {
        ...
        // Метод takeWhile() будет брать элементы из коллекции,
        // пока выполняется заданное условие
        List<String> result1 = new ArrayList<>();
        // Берем строки, длина которых меньше 6 символов
        words.stream().takeWhile(word -> word.length() < 6)
                      .forEach(e -> result1.add(e));
        ...
        // Метод takeWhile() будет брать элементы из коллекции, пока
        // выполняется заданное условие и завершится, когда встретит
        // элемент, не удовлетворяющий условию
        List<Integer> numbers = Arrays.asList(2, 4, 6, 7, 8, 10);
        List<Integer> result2 = new ArrayList<>();
        numbers.stream()
                // Берем четные числа из коллекции
        ...
        // Метод takeWhile() будет брать элементы из коллекции, пока
        // выполняется заданное условие, и завершится, когда встретит элемент,
        ...
        people.stream()
                // Берем людей, возраст которых меньше 30 лет
                .takeWhile(person -> person.getAge() < 30)
                .forEach(result3::add);

        System.out.println("Пример 3: takeWhile() с параметром встроенного класса");
        System.out.println("Результат: " + result3);
        // Вывод: [Person{name='John', age=25}, Person{name='Alice', age=30}]
    }
